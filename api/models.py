...
def agent_session_workspace_metadata(session_ids, *, profile="default") -> dict:
    ...
    try:
        ...
        from api.profiles import (
            _PROFILE_ID_RE, _is_isolated_profile_mode, _isolated_profile_name,
            _profiles_match,
        )

        # The general home resolver deliberately falls back for malformed names;
        # a cross-profile metadata probe must instead fail closed.
        profile = profile or "default"
        ...
        with closing(sqlite3.connect(f"{db_path.resolve().as_uri()}?mode=ro", uri=True)) as conn:
            ...
            for offset in range(0, len(wanted), 500):
                ...
                for sid, cwd, name, source in conn.execute(
                    f"SELECT id, cwd, {profile_expr}, {source_expr} FROM sessions WHERE id IN ({marks})", chunk,
                ):
